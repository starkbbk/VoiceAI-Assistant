...
**Key Features:**
- **ðï¸ Real-time Voice Transcription**: Constantly scans your voice with the Web Speech API.
- **ð¤ Automatic Question Detection**: Determines if your text is a query and auto-forwards it to the server.
- **â¡ Super-Fast Backend**: Real-time Socket.io integration to process requests and push answers.
- **ð§  OpenAI Integration**: Harnesses GPT-3.5-turbo for short, concise, specific responses.
- **ð Text-To-Speech (TTS)**: Listens to the response returned from OpenAI and reads it aloud in natural voice.
- **ð¨ Glassmorphism Design**: Pure Dark Mode, beautifully designed UI for an elegant experience.
...
cp .env.example .env
```
ð **Very Important**: Edit the `.env` file manually to insert your actual `OPENAI_API_KEY`. The server will fail requests until this is complete.
